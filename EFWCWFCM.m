function [Cluster_elem,M,W,Z]=EFWCWFCM(X,M,k,p_init,p_max,p_step,t_max,beta_memory,N,fuzzy_degree,d,landa,gama)
%
%[Cluster_elem,M,W,Z]=FWCW_FCM(X,M,k,p_init,p_max,p_step,t_max,beta_memory,N,fuzzy_degree,d,q,landa)
%
%M.Hashemzadeh, A.Golzari oskouei and M.A.Balafar, "Cluster-Weight and Maximum-Entropy-Regularized
%Feature-Weight learning in Fuzzy Clustering"
%
%Function Inputs
%===============
%
...
while 1
    ...
    O_F=object_fun(N,d,k,Cluster_elem,landa,M,fuzzy_degree,W,Z,p,X,gama);
    ...
    for j=1:k
        distance(j,:,:) = (1-exp((-1.*repmat(landa,N,1)).*((X-repmat(M(j,:),N,1)).^2)));
        dWkm(j,:) = W(1,j)^p .* (Cluster_elem(j,:).^fuzzy_degree) * reshape(distance(j,:,:),[N,d]);
        dWkm(j,:) =  exp(gama * dWkm(j,:));
    end
    
    tmp1 = zeros(k,d);
    for j=1:d
        tmp2 = dWkm./repmat(dWkm(:,j),1,d);
        tmp2(tmp2==inf)=0;
        tmp2(isnan(tmp2))=0;
        tmp1=tmp1+tmp2;
    end
